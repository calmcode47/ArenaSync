# ArenaFlow — Fullstack Sports Venue Management

## 🎮 Demo Access (Hackathon Judges)

> **Frontend / Web App:** [https://flowarena-694a7.web.app](https://flowarena-694a7.web.app)
> 
> **Backend / API Health:** [https://arenaflow-backend-g4fkebhsiq-ew.a.run.app/health](https://arenaflow-backend-g4fkebhsiq-ew.a.run.app/health)

### Step 1: Open the app
The VenueSelector modal will appear on first visit.

### Step 2: Enter the demo Venue ID
```
[VENUE_ID_PLACEHOLDER]
```
*(Venue ID is printed when you run `python -m scripts.seed_demo`)*

### Step 3: Log in as Staff (to test alert dispatch)
Click "STAFF LOGIN" in the VenueSelector modal:
- **Email:** `demo@arenaflow.app`
- **Password:** `ArenaFlow2026!`

### What you can test
- **Dashboard** — Live 3D crowd visualization + zone grid
- **Live Map** — Google Maps heatmap with zone markers
- **Queue Intelligence** — Queue forecasting and congestion intelligence per zone
- **Alerts Center** — Dispatch an alert (try "OVERCROWDING" + "HIGH" severity)
- **About** — Scrollytelling 3D presentation
