...
@router.post(
    "/snapshot", response_model=CrowdSnapshotOut, status_code=status.HTTP_201_CREATED
)
...
async def get_venue_summary(
    request: Request, venue_id: UUID, db: AsyncSession = Depends(get_db)
) -> Any:
...
async def get_zone_history(
    zone_id: UUID, hours: int = 24, db: AsyncSession = Depends(get_db)
) -> Any:
...
async def predict_zone_congestion(
    request: Request, zone_id: UUID, db: AsyncSession = Depends(get_db)
) -> Any:
    ...
    if not snapshots:
        raise HTTPException(
            status_code=404, detail="Insufficient historical data for prediction"
        )
