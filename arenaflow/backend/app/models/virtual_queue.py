...
from sqlalchemy import Enum as SQLEnum, Integer, String, DateTime, ForeignKey, Index
...
class VirtualQueueEntry(Base):
    ...
    status: Mapped[str] = mapped_column(
        SQLEnum("waiting", "called", "serving", "completed", "abandoned", name="vqueue_status_enum"), 
        default="waiting", 
        nullable=False
    )
    ticket_code: Mapped[str] = mapped_column(String(6), unique=True, index=True, nullable=False)
    estimated_call_time: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    ...
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.utcnow(), onupdate=lambda: datetime.utcnow(), nullable=False)
