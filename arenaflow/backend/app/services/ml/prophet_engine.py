...
try:
    ...
    ModuleNotFoundError
):  # pragma: no cover - optional dependency in lightweight test envs
...
class ProphetEngine:
    ...
    def fit(self, zone_id: str, df: pd.DataFrame) -> None:
        ...
        if len(df) < 10:
            self.logger.warning(
                f"Insufficient data (n={len(df)}) to fit Prophet for zone {zone_id}. Using fallback."
            )
    ...
    def predict(
        self, zone_id: str, periods: int = 6, freq: str = "5min"
    ) -> Dict[str, Any]:
    ...
    def fit_and_predict(
        self, zone_id: str, df: pd.DataFrame, periods: int = 6
    ) -> Dict[str, Any]:
        self.fit(zone_id, df)
