...
```ts
import { useListAllEvents, useGetEventDetails, useCreateAlert, useGetMyZoneTelemetry } from '@dataconnect/generated/react';
// The types of these hooks are available in react/index.d.ts

const { data, isPending, isSuccess, isError, error } = useListAllEvents();

const { data, isPending, isSuccess, isError, error } = useGetEventDetails(getEventDetailsVars);

const { data, isPending, isSuccess, isError, error } = useCreateAlert(createAlertVars);

const { data, isPending, isSuccess, isError, error } = useGetMyZoneTelemetry(getMyZoneTelemetryVars);
